// api/uebergabe-setup.js
// Nur für eingeloggte Abonnenten: legt Code+PIN für den Übergabebuch-Zugang fest oder ändert die PIN.
// Die PIN wird NIE im Klartext gespeichert, nur als Hash (Code als Salt) — derselbe Algorithmus
// wie in api/uebergabe-public.js, damit der spätere Vergleich dort funktioniert.
...
export default async function handler(req, res) {
  ...
  try {
    const existingRes = await fetch(
      ...
      '&tool_name=eq.uebergabe-zugang&select=id&order=updated_at.desc',
      { headers: sbHeaders() }
    );
    ...
    if (existingRows && existingRows.length > 0) {
      ...
      await fetch(SUPABASE_URL + '/rest/v1/user_tool_data', {
        ...
        body: JSON.stringify({ user_id: email, tool_name: 'uebergabe-zugang', data })
      });
    }
